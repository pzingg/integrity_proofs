defmodule DidServerWeb.CoreComponents do
  @moduledoc """
  ...
  # Not implemented (to eliminate JS dependency):

  # <.modal/>, <.flash/>, <.flash_group/>
  ...
  ## Examples
      ...
      <.button_cancel>Send!</.button_cancel>
      <.button_cancel phx-click="go" class="ml-2">Send!</.button_cancel>
  """
  attr :type, :string, default: nil
  attr :class, :string, default: nil
  attr :rest, :global, include: ~w(disabled form name value)
  ...
  def button_cancel(assigns) do
    ~H"""
    <button
      type={@type}
      class={[
        "phx-submit-loading:opacity-75 rounded-lg bg-white hover:bg-zinc-100 py-2 px-3",
        "text-sm font-semibold leading-6 border-solid border-red-500 border-2 text-red active:text-red/80",
        @class
      ]}
      {@rest}
    >
  ...
  def translate_error({msg, opts}) do
    ...
    #   Gettext.dngettext(DidServerWeb.Gettext, "errors", msg, msg, count, opts)
    # else
    #   Gettext.dgettext(DidServerWeb.Gettext, "errors", msg, opts)
    # end
